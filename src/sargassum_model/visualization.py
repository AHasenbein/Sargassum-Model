...
def mode_profit_bar(results: List[Dict[str, Any]]) -> go.Figure:
    df = pd.DataFrame(
        [
            {
                "mode": r["mode"],
...
def profit_waterfall(result: Dict[str, Any]) -> go.Figure:
...
def process_sankey(result: Dict[str, Any]) -> go.Figure:
    process = result["process"]
    wet_in = process["wet_tons_per_day"]
    dry = process["dry_tons_per_day"]
    methane = max(process["methane_mmbtu_per_day"], 0.0)
    losses = max(process["gross_energy_mmbtu_per_day"] - methane, 0.0)

    # Mixed units are intentionally separated by labels for quick visual intuition.
    fig = go.Figure(
        data=[
            go.Sankey(
                node=dict(
                    ...
                    thickness=18,
                    label=[
                        "Wet sargassum input (t/day)",
                        "Dry solids (t/day)",
                        "Converted methane energy (MMBtu/day)",
                        "Parasitic and conversion losses (MMBtu/day)",
                    ],
                ),
                link=dict(
                    source=[0, 1, 1],
                    target=[1, 2, 3],
                    value=[wet_in, methane, losses if losses > 0 else 0.001],
                ),
            )
        ]
    )
